...
interface SpotifyItem extends SpotifyPick {
  uri: string;
}

interface Props {
  value: SpotifyPick | null;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!session) return;
        const url = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/spotify-search?q=${encodeURIComponent(debounced)}`;
      ...
      <div className="flex items-center gap-3 rounded-md bg-card p-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="truncate text-sm text-muted-foreground">
            {value.subtitle}{" "}
            <span className="text-xs uppercase">· {value.type}</span>
          </div>
        </div>
        <button
          type="button"
